...
const PollTrends = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3000/polltrends/")
      .then(result => {
    ...
    axios.get("http://localhost:3000/results")
      .then(result => {
        setOverallScore({
          labels: result.data.data.map(entry => entry.voting_choice ? "False" : "True"),
          datasets: [{
            label: 'Overall Score',
  ...
  useEffect(() => {
    axios.get(`http://localhost:3000/counts?voting_choice=${votingChoice}`)
      .then(result => {
        setChartData({
  ...
  return (
    <div className='container'>
      ...
      <div className='container bg-white bg-2 text-center p-3 mb-4'>
        ...
        <button onClick={() => setVotingChoice('true')} className='btn btn-outline-success mt-4 mb-4'>Show True Votes</button>
        <button onClick={() => setVotingChoice('false')} className='btn btn-outline-danger mt-4 ms-3 mb-4'>Show False Votes</button>
